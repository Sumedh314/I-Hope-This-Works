...
class Drive : public Point {
    ...
    // Removed: IMU and rotation sensors
    // pros::IMU& inertial;
...
public:
    Drive(
        ...
        const double MOTOR_GEAR_TEETH, const double WHEEL_GEAR_TEETH, const double TRACKING_WHEEL_DIAMETER,
        ...
        pros::Controller& controller, PID drive_pid_IME, PID drive_pid, PID turn_pid
    );
    ...
    void drive_distance(double target, double max_voltage = 127);  // ADD THIS DECLARATION
    ...
    void IMU_odometry();
    void reset_odometry();
    void set_original_heading(double original_heading);  // ADD THIS DECLARATION
    double get_heading();
};
